...
async function main() {
  ...
  const typeormCmmandArgs = [
    'typeorm',
    '-d',
    './libs/database/src/datasources/migration.datasource.ts',
    'migration:generate',
    `./libs/database/src/migrations/${migrationName}`
  ];
  ...
  await buildCommandPromise('yarn', typeormCmmandArgs);
